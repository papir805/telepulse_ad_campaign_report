...
# %%
new_df = purchase_data.iloc[3:,:].transpose()
#new_df.head()
...
# %%
new_df = new_df.drop(labels=3, axis=1)

# %%
new_df.columns = new_df.iloc[0]

# %%
new_df = new_df.drop(labels='Source')
...
# %%
sum_of_purchases = new_df.sum(axis=0)
#sum_of_purchases
...
# %%
new_df_groupby_date = new_df.groupby(pd.Grouper(freq='M')).agg('sum')
new_df_groupby_date

# %%
new_df_grouped_transpose = new_df_groupby_date.transpose()
new_df_grouped_transpose = new_df_grouped_transpose.stack().to_frame()
new_df_grouped_transpose.rename(columns={0:'Purchases'}, inplace=True)
new_df_grouped_transpose
...
# %%
# joined_tbl = lookup_data[['Exit Survey', 'Airings']].merge(right=new_df_grouped_transpose, left_on='Exit Survey', right_on='Source', how='left')
# #joined_tbl.drop(labels='Exit Survey', axis=1, inplace=True)
# joined_tbl

# %%
purchase_grouped = new_df_grouped_transpose.join(lookup_data, how='left')
purchase_grouped
